...
<div class="col-md-12 col-sm-12 text-right">
	<div class="dropdown">
		<a class="btn btn-info" href="{{url('/pertemuan/tambah/')}}" role="button">Tambah Data</a>
...
<table class="data-table table stripe hover">
	<form>
		<thead>
			{{csrf_field()}}
			<tr>
				<th class="table-plus datatable-nosort">NO PERTEMUAN</th>
				<th>Peserta</th>
				<th>ID TAHUN AJAR</th>
				<th>ID KELAS</th>
				<th>ID PEGAWAI</th>
		...
		<tbody>
		 ...
		 <tr>
		 	<td class="table-plus">{{$per->NO_PERTEMUAN}}</td>
			<td>{{$per->peserta->NAMA_PESERTA}}</td>
			<td>{{$per->tahunajar->TAHUNAJAR}}</td>
			<td>{{$per->kelas->NAMA_KELAS}}</td>
			<td>{{$per->pegawai->NAMA_PEG}}</td>
			<td>{{$per->JUMLAH_PERTEMUAN}}</td>
			<td>
				<!-- <a class="dropdown-item" href="#"><i class="dw dw-edit2"></i> Edit</a>
				<a class="dropdown-item" href="#"><i class="dw dw-delete-3"></i> Delete</a> -->
				<a class="" href="{{url('/pertemuan/edit/'.$per->NO_PERTEMUAN)}}"><i class="dw dw-edit2"></i> Edit | </a>
				<a class="" href="{{url('/pertemuan/hapus/'.$per->NO_PERTEMUAN)}}" onclick="return confirm('apakah anda yakin untuk menghapus?')"><i class="dw dw-delete-3"></i> Delete</a>
			</td>
		</tr>
		@endforeach
	</tbody>
